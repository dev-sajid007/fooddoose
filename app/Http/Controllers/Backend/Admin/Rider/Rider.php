...
{
public function show_rider(){
return view('admin.extends.riders.manage_rider');
}



public function get_all_riders(){
$query = DB::table('users')->where('user_type','rider')
...
return response()->json($data);
    ...
    {
        Order::where('id', $request->order_id)->update(['rider_id' => $request->rider_id, 'status' => 'pending']);

        return back()->with('message_success', 'Driver has been assigned successfully!');
    }
}
